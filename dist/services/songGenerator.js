...
function guitarWave(frequency, t) {
    ...
    return (Math.sin(2 * Math.PI * frequency * t) * 1.00 +
        ...
        Math.sin(2 * Math.PI * frequency * 1.002 * t) * 0.30) * envelope * 0.30;
}
function padWave(frequency, t) {
    if (t < 0)
        return 0;
    const envelope = Math.min(1, t / 0.15) * Math.exp(-t / 3.0);
    return (Math.sin(2 * Math.PI * frequency * t) * 1.00 +
        Math.sin(2 * Math.PI * frequency * 2 * t) * 0.20 +
        Math.sin(2 * Math.PI * frequency * 0.5 * t) * 0.15) * envelope * 0.25;
}
function bassWave(frequency, t) {
    if (t < 0)
        return 0;
    const envelope = Math.min(1, t / 0.005) * Math.exp(-t / 0.5);
    return (Math.sin(2 * Math.PI * frequency * t) * 1.00 +
        Math.sin(2 * Math.PI * frequency * 2 * t) * 0.30 +
        Math.sin(2 * Math.PI * frequency * 3 * t) * 0.10) * envelope * 0.50;
}
function kickDrum(t) {
    if (t < 0 || t > 0.4)
        return 0;
...
function hihat(t, noiseVal, open) {
    if (t < 0)
        return 0;
...
function makeNoiseBuffer(size, rng) {
...
export function generateWav(seed) {
    ...
    const melodyPerChord = progression.map(chord => {
        const extended = [
            chord[0] * 0.944,
            chord[0],
            chord[1],
        ...
        return Array.from({ length: 8 }, () => extended[Math.floor(rng() * extended.length)]);
    });
    const bassPattern = Array.from({ length: 8 }, (_, i) => i === 0 || i === 4 ? 1 : rng() > 0.6 ? 1 : 0);
    ...
    for (let ci = 0; ci < fullProgression.length; ci++) {
        ...
        for (let i = 0; i < samplesThisChord; i++) {
            ...
            const chordSmp = (chordFn(chord[0], t) +
                chordFn(chord[1], t) +
                chordFn(chord[2], t)) / 3;
            ...
            const fadeFactor = globalI > fadeStartSample
                ...
                : 1;
            samples[globalI] = (chordSmp * 0.18 +
                melodySmp * 0.32 +
                bassSmp * 0.22 +
                drumSmp * 0.28) * fadeFactor;
        }
    }
    let peak = 0;
