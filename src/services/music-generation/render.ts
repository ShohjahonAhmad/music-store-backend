...
function unitsToTicks(units: number): number {
    return Math.round(units * TICKS_PER_UNIT);
}

const NOTE_NAMES = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];

function midiNumberToName(midiNumber: number): string {
...
export function renderToMidiBuffer(composition: Composition, rng: () => number): Buffer {
    ...
    const melodyProgram = pick(rng, GM.melodyLead as unknown as number[]);
    const bassProgram = pick(rng, GM.bass as unknown as number[]);
    const chordsProgram = pick(rng, GM.chordsPad as unknown as number[]);
    const arpProgram = pick(rng, GM.arp as unknown as number[]);
