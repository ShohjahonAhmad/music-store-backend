...
let soundBankPromise: Promise<any> | null = null;
...
export async function midiToWav(midiBuffer: Buffer, signal : AbortSignal): Promise<Buffer> {

    const midi = BasicMIDI.fromArrayBuffer(
        Uint8Array.from(midiBuffer).buffer
      );
