...
export function planSong(rng: () => number): SongPlan {
    ...
    const sectionTypes: SectionType[] = ["intro", "verse", "chorus", "verse", "chorus", "bridge", "chorus", "outro"];
    const sectionBars: Record<SectionType, number> = { intro: 2, verse: 4, chorus: 4, bridge: 2, outro: 2 };

    let plan: { type: SectionType; bars: number }[] = sectionTypes.map((type) => ({ type, bars: sectionBars[type] }));
    ...
    const progByType: Record<SectionType, typeof verseProg> = {
        verse: verseProg,
        chorus: chorusProg,
        bridge: bridgeProg,
        intro: verseProg,
    ...
    const sections: SongSection[] = plan.map((p) => {
...
export function composeSong(rng: () => number): Composition {
    ...
    const tracks: Composition["tracks"] = { melody: [], bass: [], chords: [], arp: [], drums: [] };
    let barOffset = 0;
